...
int main(int argc, char* argv[])
{
  ...
  ros::Publisher pub = n.advertise<std_msgs::Float32MultiArray>("/varun/ip/buoy", 1000);
  ...
  while (1)
  {
    ...
    cv::findContours(thresholded_Mat, contours, CV_RETR_TREE, CV_CHAIN_APPROX_SIMPLE);  // Find contours in image
    ...
    if (((320 - center[0].x > -5) && (320 - center[0].x < 5)) && ((240 - center[0].y > -5) && (240 - center[0].y) < 5))
