...
{
  ros::init(argc, argv, "octagon_detection");
  ros::NodeHandle n;
  ros::Publisher robot_pub = n.advertise<std_msgs::Bool>("/varun/ip/octagon_detection", 1000);
  ros::Subscriber sub = n.subscribe<std_msgs::Bool>("octagon_detection_switch", 1000, &octagonDetectedListener);
  ...
  while (ros::ok())
  {
    ...
    if (!IP)
    {
      ...
      if (alert == 1)
      {
        ...
        if (oldAlert != alert)
        {
          ROS_INFO("%s: found octagon", ros::this_node::getName().c_str());
          oldAlert = alert;
        }
      }
      else if (alert == 0)
      {
        ...
        if (oldAlert != alert)
        {
          ROS_INFO("%s: no octagon", ros::this_node::getName().c_str());
          oldAlert = alert;
        }
      }
      else
